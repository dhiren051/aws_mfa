...
parser.add_argument('--profile', help='aws profile to store the \
    session token', default=os.getenv('AWS_PROFILE'))
parser.add_argument('--arn', help='AWS ARN from the IAM console (Security \
    credentials -> Assigned MFA device). This is saved to your \
    .aws/credentials file')
parser.add_argument('--credential-path', help='path to the aws credentials \
    file', default=os.path.expanduser('~/.aws/credentials'))

args = parser.parse_args()


if args.profile is None:
    parser.error('Expecting --profile or profile set in environment \
    AWS_PROFILE. e.g. "stage"')
...
# Generate the session token from the default profile based on the environment.
# We do not want to overwrite these profiles as we wouldn't
# be able to generate another token
result = subprocess.run([
    'aws', 'sts', 'get-session-token',
    '--profile', args.profile + '-default', '--serial-number',
    args.arn, '--token-code', args.token], stdout=subprocess.PIPE,
